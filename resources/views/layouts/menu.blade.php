...
<li class="dropdown">
    ...
    <ul class="dropdown-menu">
        @foreach (OpcionesExtrasController::getBloques($encuesta->id_encuesta) as $bloque)
            <li><a href="{{ route('getRespuestas', ['id_encuesta' => $encuesta->id_encuesta, 'id_bloque' => $bloque->id_bloque, 'punto' => $bloque->c_nombre_bloque]) }}"
                    class="nav-link text-white miclasey">{{ $bloque->c_nombre_bloque }}</a></li>
        @endforeach
    </ul>
</li>
@endforeach
